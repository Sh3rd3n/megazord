...
export function registerMilestoneCommands(parent: Command): void {
	const milestone = parent
		.command("milestone")
		.description("Milestone lifecycle operations");
	...
	milestone
		...
		.requiredOption(
			"--phases <string>",
			"Comma-separated phase numbers (e.g., 1,2,3)",
		)
	...
	milestone
		.command("archive")
		.description(
			"Archive a milestone: copy roadmap/requirements, create git tag",
		)
	...
	milestone
		...
		.requiredOption(
			"--phases <string>",
			"Comma-separated phase numbers to audit (e.g., 1,2,3)",
		)
